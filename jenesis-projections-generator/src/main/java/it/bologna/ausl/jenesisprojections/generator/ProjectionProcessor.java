...
public class ProjectionProcessor extends AbstractProcessor {
    ...
    private void generateJavaCode() throws IOException, ClassNotFoundException {
    ...
    private void discoverEntitiesAndGenerate(VirtualMachine vm) throws ClassNotFoundException, IOException {
    ...
    private void readEntityAndCreateProjections(Element entity, VirtualMachine vm) throws IOException, ClassNotFoundException {

        List<Element> plainFields = new ArrayList<>();
        List<Element> fieldsToExpand = new ArrayList<>();
...
//                        System.out.println("found foreign key field: " + enclosedElement);
                    fieldsToExpand.add(enclosedElement);
//                        System.out.println("-------------");
                }
            }
        }
        ...
        fieldsToExpand.sort((Element f1, Element f2) -> {
            PackageElement f1Package = processingEnv.getElementUtils().getPackageOf(f1);
            PackageElement f2Package = processingEnv.getElementUtils().getPackageOf(f2);
            String f1Name = f1Package.getQualifiedName().toString() + "." + f1.getSimpleName();
            String f2Name = f2Package.getQualifiedName().toString() + "." + f2.getSimpleName();
        ...
        String projectionClassName = generateProjectionClassWithPlainFields(vm, entity, plainFields, fieldsToExpand);
        discoverAndCreateProjections(vm, entity, projectionClassName, null, fieldsToExpand);
    }
